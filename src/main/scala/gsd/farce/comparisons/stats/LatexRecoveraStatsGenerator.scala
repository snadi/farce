 ...
 */
object LatexRecoveraStatsGenerator {
  ...
  def main(args: Array[String]) {
    ...
    for (system <- Utilities.getSupportedSystems) {
      if (system != "default") {
        ...
        hierarchyStats.readStats(Utilities.getSystemConfig(system).getHierarchyResults)
        hierarchyStats.outputStats
        hierarchySystemStats += hierarchyStats


        val crosstreeStats = new ComparisonStat(system + "Crosstree")
        crosstreeStats.readStats(Utilities.getSystemConfig(system).getCrosstreeResults)
