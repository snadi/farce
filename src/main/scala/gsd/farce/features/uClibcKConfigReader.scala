 ...
 *
 * [TODO] remove and use LVAT Kconfig reader
 *
 */

object uClibcKConfigReader extends App {

  if (args.size < 1) {
    println("expected four (3rd optional) parameters\n" +
      " - directory (which contains a Config.in file)\n" +
      " - target file in which the feature model is stored\n" +
      " - target file in which the generated header file is stored\n" +
  ...
  def processConfig() {
    ...
    if (flagType == "bool") {
      ...
      if (!flagDep.isEmpty) {
        outFeatureModel += "defined(__" + flag + "__) => " + flagDep.reduceLeft("(" + _ + ") && (" + _ + ")").replaceAll("\\w+", "defined(__$0__)") + "\n"
        // flagDep.map(dependency => outFeatureModel += "defined(__" + flag + "__) => " + dependency.replaceAll("\\w+", "defined(__$0__)") + "\n")
      }
      if (!groupDep.trim.isEmpty)
        outFeatureModel += "defined(__" + flag + "__) => " + groupDep.replaceAll("\\w+", "defined(__$0__)") + "\n"
  ...
  def processChoice() {
    ...
    outFeatureModel += choiceAlternatives.map("defined(__" + _ + "__)").mkString("oneOf(", ",", ")\n")
  }
  ...
  while (!config.isEmpty) {
    ...
    if (!skip && line.startsWith("menuconfig")) {
      println("<menuconfig " + line.drop(11) + ">")
      inChoiceHeader = false
      processConfig()
      flag = line.drop(11).trim
    } else if (!skip && line.startsWith("menu")) {
    ...
    if (!skip && line.startsWith("source")) {
      processConfig()
      var file = (path + line.substring(line.lastIndexOf("/")).replaceAllLiterally("\"", ""))
    ...
    if (!skip && !skipHelp && (flag != "" || inChoiceHeader) && line.trim() == "help") {
      skipHelp = true
    }
  ...
  if (args.size > 3) {
    ...
    try {
      for (feature <- features) p.write("__" + feature + "__\n")
    } finally {
      p.close()
    }
  }
